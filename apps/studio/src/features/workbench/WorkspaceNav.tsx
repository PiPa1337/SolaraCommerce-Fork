...
export function WorkspaceGroups({ label, groups }: { label: string; groups: WorkspaceGroup[] }) {
  ...
  return <div className="workspace-groups" ref={root}>
    ...
    {groups.map((group) => <section key={group.id} id={`${prefix}-${group.id}`}
      tabIndex={-1} className="workspace-group" aria-label={group.label}>
