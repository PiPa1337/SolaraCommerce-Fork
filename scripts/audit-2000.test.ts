...
test("audit 2000 desglose", () => {
  ...
  const files = [...result.files.entries()].map(([p,v])=>[p, typeof v==="string"? Buffer.byteLength(v,"utf8"): (v as Uint8Array).byteLength] as const);
  let total=0; for(const [,b] of files) total+=b;
  ...
  for(const [p,b] of files){
    ...
    byCat[c].count++; byCat[c].bytes+=b; byCat[c].files.push([p,b]);
  }
  for(const [c,info] of Object.entries(byCat).sort((a,b)=>b[1].bytes-a[1].bytes)){
    console.log(c, info.count, info.bytes, (info.bytes/total*100).toFixed(1)+"%", "avg",Math.round(info.bytes/info.count));
