...
log.on('error', err => { console.error('log error', err); });
console.log('[nightwatch-run] log created');
const proc = spawn(cmd, { stdio: ['ignore', 'pipe', 'pipe'], shell: true });
proc.stdout.on('data', d => { log.write(d); process.stdout.write(d); });
proc.stderr.on('data', d => { log.write(d); process.stderr.write(d); });
proc.on('close', code => {
