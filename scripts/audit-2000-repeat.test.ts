...
test("audit repetido 2000", () => {
  ...
  let suffix=0;
  for(let i=1;i<min;i++) if(a[a.length-i]===b[b.length-i]) suffix++; else break;
  console.log("producto HTML len",a.length, b.length, "prefijo comun",common, "sufijo comun",suffix, "unico aprox",a.length-common-suffix);
  ...
  console.log("catalog-index", Buffer.byteLength(String(result.files.get("catalog-index.json")),"utf8"));
  console.log("search-index", Buffer.byteLength(String(result.files.get("search-index.json")),"utf8"));
});
