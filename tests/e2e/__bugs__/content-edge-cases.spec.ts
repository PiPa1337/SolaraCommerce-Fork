...
function mutated(overrides: Partial<StoreProjectV1> & { productOverrides?: Array<Record<string, unknown>> }): StoreProjectV1 {
  const clone = JSON.parse(JSON.stringify(catalogModernV2Store)) as StoreProjectV1 & { productOverrides?: unknown };
...
async function serve(project: StoreProjectV1): Promise<void> {
  ...
  server = createServer((request, response) => {
    ...
    response.writeHead(200); response.end(file);
  });
...
test("exporta y renderiza con textos extremos", async ({ page }) => {
  ...
  test("precios extremos se formatean sin romper layout", async ({ page }) => {
    await serve(mutated({
      productOverrides: [
        { variants: [{ id: "v1", title: "U", price: 1, available: true, optionValues: {}, sku: "A", stockStatus: "in_stock" }] },
        { variants: [{ id: "v2", title: "U", price: 9999999999, available: true, optionValues: {}, sku: "B", stockStatus: "in_stock" }] },
      ],
    }));
    await page.goto(`${serverUrl}/`);
    const overflow = await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
  ...
  test("productos sin imagenes no rompen la grilla", async ({ page }) => {
    ...
    await page.goto(`${serverUrl}/`);
    const imgs = await page.evaluate(() => [...document.querySelectorAll(".catalog-product-card img")].filter((img) => !img.getAttribute("src")));
