...
test.beforeAll(async () => {
  server = createServer((request, response) => {
    ...
    response.writeHead(200, { "Content-Type": path.endsWith(".css") ? "text/css" : "text/html; charset=utf-8" });
    response.end(file);
  });
...
async function openCheckoutDrawer(page: import("@playwright/test").Page, payload: string): Promise<void> {
...
test("campos de 10k caracteres y solo espacios no rompen el preview del pedido", async ({ page }) => {
  test.setTimeout(60000);
  ...
  const previewText = await page.evaluate(() => document.querySelector("[data-order-preview]")?.textContent ?? "");
