...
test.beforeAll(async () => {
  server = createServer((request, response) => {
    ...
    if (requested === "catalog-index.json") { response.writeHead(500).end("{corrupto"); return; }
...
test("localStorage bloqueado: el sitio sigue navegable", async ({ browser }) => {
  ...
  await expect(page.locator("[data-product], [data-solara-module*=product]").first()).toBeVisible({ timeout: 10000 });
  expect(errors.filter((e) => !e.includes("storage")), `errores JS: ${errors.join(" | ")}`).toHaveLength(0);
  await context.close();
});
...
test("imagenes rotas: layout sin overflow y alt presente", async ({ page }) => {
  ...
  const missingAlt = await page.evaluate(() => [...document.querySelectorAll("img")].filter((img) => img.getAttribute("alt") === null).length);
