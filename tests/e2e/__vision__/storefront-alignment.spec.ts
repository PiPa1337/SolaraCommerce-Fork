...
test.beforeAll(async () => {
  server = createServer((request, response) => {
    ...
    response.writeHead(200, { "Content-Type": extension === "css" ? "text/css; charset=utf-8" : "text/html; charset=utf-8", "Cache-Control": "no-store" });
    response.end(file);
  });
...
test.describe("alineacion storefront exportado", () => {
  test("cards de producto comparten ancho y alto dentro de la grilla", async ({ page }) => {
    ...
    await expect(page.locator('[data-solara-module="catalog-product-grid"]').first()).toBeVisible({ timeout: 10000 });
    ...
    const grids = await page.evaluate(() => {
      return [...document.querySelectorAll('[data-solara-module="catalog-product-grid"]')].map((grid) => {
  ...
  test("secciones del home usan contenedor centrado consistente", async ({ page }) => {
    ...
    const modules = await page.evaluate(() => {
      return [...document.querySelectorAll("[data-solara-module]")].map((el) => {
        const r = el.getBoundingClientRect();
