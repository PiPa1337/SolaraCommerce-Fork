  ...
  const addEntry = (
    ...
    entries.push({
      ...
      ...(source.kind === "section-repeater-item"
        ? {
            itemIds: Array.isArray((section.settings as Record<string, unknown>)[source.fieldKey])
              ? ((section.settings as Record<string, unknown>)[source.fieldKey] as unknown[])
                  .map((item) =>
