...
> = {
  ...
  render(context) {
    ...
    return moduleRoot(
      ...
      context.section,
      safeHtml(
        `<div class="catalog-footer-inner" data-motion-zone="content"><div class="catalog-footer-brand"><a class="catalog-brand" href="/">${renderBrand(context.project)}</a><p>${escapeHtml(note)}</p></div><nav aria-label="${escapeAttribute(copy.footer.explore)}"><strong>${escapeHtml(copy.footer.explore)}</strong><a href="/">${escapeHtml(copy.navigation.home)}</a>${catalogLinkMarkup}${searchLink}</nav><nav aria-label="${escapeAttribute(copy.footer.help)}"><strong>${escapeHtml(copy.footer.help)}</strong>${helpPageLinks}${policyLinks}</nav><address><strong>${escapeHtml(copy.footer.contact)}</strong>${contact}</address><div class="catalog-footer-meta"><small>© ${new Date().getUTCFullYear()} ${escapeHtml(context.project.identity.brandName)}. ${escapeHtml(copy.footer.copyright ?? "Todos los derechos reservados.")}</small><p class="catalog-footer-made"><a href="https://solara.com.ar" target="_blank" rel="noopener noreferrer">Hecho con ❤️ en solara.com.ar</a></p></div></div>`,
      ),
      { tag: "footer" },
    );
  },
