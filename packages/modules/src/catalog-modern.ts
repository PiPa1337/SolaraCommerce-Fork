  ...
  render(context) {
    ...
    return moduleRoot(
      ...
      safeHtml(`<div class="catalog-header-inner" data-motion-zone="content">
        ...
        <noscript><style>[data-solara-store].catalog-modern .catalog-search-link{display:none}[data-solara-store].catalog-modern .catalog-search-noscript{display:inline-flex;align-items:center;min-height:44px;padding:.55rem 1rem;border:1px solid var(--catalog-border);border-radius:0;background:var(--catalog-surface);color:var(--catalog-muted);text-decoration:none;font-size:.82rem}@media (max-width:767px){[data-solara-store].catalog-modern .catalog-mobile-menu[hidden]{display:block}}@media print{[data-solara-store].catalog-modern .catalog-mobile-menu{display:none!important}}</style></noscript>
...
> = {
  ...
  render(context) {
    ...
    return moduleRoot(
      ...
      context.section,
      safeHtml(
        `<div class="solara-cart-backdrop catalog-cart-backdrop" data-solara-cart-close data-close-cart hidden></div><aside id="solara-cart" class="catalog-cart-drawer" data-cart-drawer role="dialog" aria-label="${escapeAttribute(context.settings.title)}" aria-modal="true" aria-hidden="true" inert tabindex="-1"><header><h2${canvasTextAttributes(canvasContext(context), "title", 120)}>${escapeHtml(context.settings.title)}</h2><button type="button" data-solara-cart-close data-close-cart aria-label="${escapeAttribute(copy.cart.close)}">${escapeHtml(copy.navigation.close)}</button></header><div class="catalog-cart-scroll"><div class="catalog-cart-items" data-solara-cart-items data-cart-lines><p class="catalog-empty"${canvasTextAttributes(canvasContext(context), "emptyText", 240)}>${escapeHtml(context.settings.emptyText || copy.empty.cart)}</p></div><div class="catalog-cart-summary"><p><span>${escapeHtml(copy.cart.subtotal)}</span><strong data-cart-subtotal aria-live="polite">${escapeHtml(formatMoneyForProject(0, context.project))}</strong></p><p><span>${escapeHtml(copy.cart.delivery)}</span><strong>${escapeHtml(copy.cart.deliveryToCoordinate)}</strong></p><p class="catalog-cart-total"><span>${escapeHtml(copy.cart.estimatedTotal)}</span><strong data-solara-cart-total data-cart-total aria-live="polite">${escapeHtml(formatMoneyForProject(0, context.project))}</strong></p></div><form class="catalog-checkout-form" data-solara-checkout data-checkout-form id="catalog-drawer-checkout"><label for="catalog-drawer-name">${escapeHtml(copy.cart.name)}</label><input id="catalog-drawer-name" name="name" autocomplete="name" required><label for="catalog-drawer-phone">${escapeHtml(copy.cart.phone)}</label><input id="catalog-drawer-phone" name="phone" autocomplete="tel" inputmode="tel" pattern="[0-9+ ()-]{8,}" title="Ingresá un teléfono válido" required><label for="catalog-drawer-address">${escapeHtml(copy.cart.address)}</label><textarea id="catalog-drawer-address" name="address" autocomplete="street-address" required></textarea><label for="catalog-drawer-notes">${escapeHtml(copy.cart.notes)}</label><textarea id="catalog-drawer-notes" name="notes"></textarea><pre data-order-preview aria-live="polite" role="status"></pre></form></div><div class="catalog-drawer-footer"><button class="catalog-primary-action" type="submit" form="catalog-drawer-checkout"${canvasTextAttributes(canvasContext(context), "checkoutLabel", 120)}>${escapeHtml(context.settings.checkoutLabel)}</button>${checkoutVerificationMarkup}</div></aside>`,
      ),
    );
  },
};
