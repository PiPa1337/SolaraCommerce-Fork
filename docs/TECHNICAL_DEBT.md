...
| P3 | Abierto (traza T5): `autosave.dispose()` vive en el cleanup de un efecto re-ejecutable (`Studio.tsx:469-481`): en modo managed cada cambio de `managedDirty` descarta la cola (hoy inofensivo porque la cola nunca se agenda en managed). | Descarte de la cola en efectos intermedios si algún día se agenda en managed. | Mover `dispose()` a un efecto de desmontaje puro. |
| P3 | Abierto (traza T19): Ctrl+S sigue activo con el shell `inert` durante un conflicto de versión: `save()` reintentaría y volvería a reportar `VERSION_CONFLICT` (el botón sí queda inaccesible por inert). | Reintentos redundantes del atajo durante un conflicto. | Gatear el atajo con el estado de conflicto o aceptar el reintento inofensivo. |
| P3 | Abierto (traza T1): settings muertas `showRating` (catalog-product-grid) y `example` (testimonios): el editor escribe y el schema valida, pero el renderer del módulo nunca las lee; el toggle no cambia el sitio. | Superficie muerta que confunde lectores y promete efectos inexistentes. | Decidir el markup de valoración en cards y el filtro de ejemplos (decisión de producto), o quitarlas con test. |
