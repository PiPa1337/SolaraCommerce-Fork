...
| P2 | Parcialmente resuelto (2026-08-23): draft distingue su runtime con la marca `// DEBUG: modo draft`, production permanece inline byte-idéntico, `scripts/build-runtime.mjs` genera bundle+mapa local y `check:runtime-serialization` ahora valida que el entry draft compila exponiendo `storefrontBoot` + `solaraReady`. Falta emitir `assets/storefront.js.map` desde el export draft. Tests: `scripts/runtime-serialization.test.ts`, `packages/exporter/src/runtime-debug.test.ts` y `tests/e2e/__bugs__/runtime-draft.spec.ts`. | El mapa aún no viaja con el sitio draft; debugging requiere paso local manual. | Emitir mapa en export draft servido sólo desde preview (Task 5 restante del plan `docs/superpowers/plans/2026-08-21-flaky-e2e-runtime-debuggeable.md`). |
