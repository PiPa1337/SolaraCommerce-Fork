...
| P2 | Abierto (detectado 2026-08-21, pre-existente verificado en baseline `c4d71ae`): specs E2E inestables en smoke paralelo: `assets > procesa una imagen`, `nojs-coverage E1/C2`, `ui-sweep-a27 C4/C8/C11`, `scale-store > busca por ancestro`, `axe-site A1` y `catalog-modern-v2 > recupera el carrito / estabilidad visual` fallan intermitentemente bajo carga (8 workers) pero pasan aislados o con reintento; el mismo set falla en el commit base sin los refactors y se mantiene estable tras 5 commits de refactor (última corrida: 117 passed, 1 flaky, 8 failed — mismos specs). | Gate smoke puede fallar sin regresión real; dificulta detectar bugs nuevos. | Plan de resolución aprobado: `docs/superpowers/plans/2026-08-21-flaky-e2e-runtime-debuggeable.md` (medición → contención vía `unstable.json` → fix raíz por familia → re-inclusión verificada). Política preventiva nueva en `TESTING.md`. |
...
| P2 | Abierto (plan mejora arquitectónica 2026-08-21): runtime storefront serializado vía `fn.toString()` sin source maps: los errores del sitio exportado no son trazables al código fuente y no hay breakpoints útiles. | Debugging del sitio público lento y propenso a interpretaciones erróneas. | Plan de resolución aprobado: Tasks 5-7 de `docs/superpowers/plans/2026-08-21-flaky-e2e-runtime-debuggeable.md` (build dual esbuild: draft externo + source map servido sólo local; production inline byte-idéntico con determinism/budgets como red). |
