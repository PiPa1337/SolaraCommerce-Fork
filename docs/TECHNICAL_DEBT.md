...
| P2 | Abierto (plan mejora arquitectónica 2026-08-21): runtime storefront serializado vía `fn.toString()` sin source maps: los errores del sitio exportado no son trazables al código fuente y no hay breakpoints útiles. | Debugging del sitio público lento y propenso a interpretaciones erróneas. | Pipeline esbuild dedicado: build `dist/runtime.js` + source map para draft, inline serializado sólo para production, con test de paridad AST/behavior entre ambos. |
