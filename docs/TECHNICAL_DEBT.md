...
| P3 | Resuelto parcialmente (traza T1, 2026-08-11): `showRating` de `catalog-product-grid` ahora se consume en el renderer y muestra u oculta el resumen de reseñas visibles con cobertura de Preview, exportación y Constructor. Continúa abierta la setting `example` de testimonios: el editor escribe y el schema valida, pero el renderer todavía no la lee. | La superficie de testimonios aún puede confundir; la valoración dejó de ser un control muerto. | Mantener la regresión de `showRating`; decidir el filtro de ejemplos de testimonios antes de cambiar el contrato. |
...
| P3 | Gaps de contrato/auto-feedback documentados durante el barrido, sin fix: el tooltip duplica la burbuja CSS con el `title` nativo (fallback para AT); avisos de lote "1 duplicadas/recuperadas" en plural en algunos caminos; GTIN/MPN sin validación de formato (strings libres del schema); SKU duplicado aceptado sin feedback (diferido, ver SCH2); algunos diálogos todavía pueden devolver el foco al `body` después de confirmar; el estado colapsado de los acordeones de Overview no sobrevive al cambio de pestaña; moneda/locale sin control en Overview (literales del schema `z.literal("ARS")`/`z.literal("es-AR")`). `ProductEditor` ya restaura el foco y sus datos de preventa/404 cuentan con controles y cobertura. | Contratos menores o decisiones de producto que un futuro cambio puede confundir. | Resolver por área en barridos puntuales; las decisiones de schema (GTIN/MPN, moneda/locale) requieren aceptación explícita. |
