...
| --- | --- | --- | --- |
| P1 | La matriz release Node 24.x no se pudo ejecutar completa: este host tiene Node 24.18.0 (el runtime bundled es 24.19.0). | El build local no equivale a la validación completa exigida por CI/release. | Ejecutar `test:e2e:release` y los gates de cierre en un runner Windows con Node 24.x. |
...
| P2 | Resuelto (2026-08-28): Live Canvas declara bindings para Catalog Modern, About V2, Contact V2 y legacy; el manifest expande entidades, alt, rich text, precio entero, PDP generado y repeaters por ID. `live-canvas-coverage.spec.ts` 12/12. | La matriz Firefox/WebKit y el release completo todavía quedan fuera de este host. | Repetir la matriz completa en el runner release Node 24.x. |
...
| P3 | El release y `engines` exigen Node 24.x. | Usar otra versión puede ocultar problemas del runtime oficial. | Mantener Node 24.x en CI, release y entornos locales. |
...
| P2 | Resuelto (2026-08-23): specs E2E inestables cerrados con evidencia: 3 specs validados 10/10 con `scripts/e2e-stability.mjs`; `assets` fijado apuntando al asset subido por atributo (conteo inicial racheable); `scale-store` dividido en dos tests y su locator móvil corregido ("Menú" no existía, el summary dice "Abrir menú"). `unstable.json` queda vacío y el smoke completo (15 specs) pasa verde. Se repararon además 14 specs rotos por la migración webp `9a22a95`. | — | Vigilar specs nuevos según Política de estabilidad E2E en `TESTING.md`. |
...
| P2 | Abierto (detectado 2026-08-21): specs unitarios del exporter inestables dentro de check:quick (8 paquetes en paralelo): request-handler legacy-retiro, chaos-storage ENOSPC y redteam-persist INV1/INV2 exceden el timeout de 5s bajo carga pero pasan siempre aislados (verificado en 3 corridas). | El gate diario puede fallar sin regresion real, entrenando a ignorar el rojo. | Subir testTimeout de esos archivos a 10s o mover los lentos fuera del gate paralelo. |
...
| P2 | Resuelto parcial (2026-08-27): draft conserva la marca `// DEBUG: modo draft`, el runtime se publica con fingerprint reproducible y production permanece sin sourcemap. `check:runtime-serialization` y el E2E de runtime validan el contrato vigente. | El exporter todavía no publica un source map asociado al runtime hasheado. | Si el debugging del draft lo requiere, emitir el mapa junto con el fingerprint y actualizar el budget. |
| P2 | Abierto (plan mejora arquitectónica 2026-08-21): `exporter/src/index.ts` conserva renderDocument/buildPages/buildFiles (~1975 líneas) tras extraer html/urls/whatsapp/assets/structured-data/feeds/audit. | El núcleo de render sigue concentrando rutas y semántica, pero ya -30% desde 2797. | Continuar extracción por página (`pages/home.ts`, `pages/product.ts`, etc.) manteniendo index.ts como barrel; cada extracción con parity/determinism verdes. |
| P3 | Abierto (plan mejora arquitectónica 2026-08-21): tests ad-hoc sin convención (`fuzz`, `fuzz2`, `fuzz100`, `audit-2000-repeat`) y `mutation-killers` duplicado en 6 paquetes sin utilidades compartidas. | Duplicación de lógica de test y onboarding confuso. | Crear `packages/test-utils` (fixtures, generadores seedables, invariantes) y consolidar archivos; documentar convención en TESTING.md. |
| P3 | Abierto (plan mejora arquitectónica 2026-08-21): `HistoryState.past` ahora limitado a 50 snapshots completos (`MAX_HISTORY_LENGTH`); si el uso real con catálogos grandes exige más historial, migrar a inverse patches con structural sharing. | Undo disponible sólo para las últimas 50 acciones. | Medir uso real antes de migrar a patches; test de equivalencia de 1000 comandos si se implementa. |
...
| P2 | Resuelto (2026-08-24): baseUrl en subcarpeta — el runtime lee `data-base-href` del html y prefija los fetch de catalog-index.json y search-index.json; los hrefs ya se prefijaban via prefixDocumentHrefs e internalHref. Tests: E2E subfolder-site, exporter unitario, runtime 56/56. | — | Mantener data-base-href + baseHref como contrato. |
...
| P2 | Resuelto (2026-08-24): guard de raíz portable — instance.json guarda portableRoot; si difiere al arrancar, dialog Electron permite continuar o salir. Sin identificadores de máquina. Tests: portable.test.mjs 3/3. | — | Mantener campo portableRoot en instance.json. |
| P3 | Resuelto (decisión documentada, 2026-08-24): formato de fecha, moneda y separadores usa locale es-AR fijo. El runtime lee root.lang (inyectado por el exporter desde project.locale) pero el default del schema es es-AR. Para v1.0, el mercado objetivo es Argentina; soporte multi-locale queda diferido para v1.1+. | Tiendas fuera de Argentina verían formato argentino. | Si se necesita multi-locale, agregar selector de idioma al schema y propagar a formatPrice/formatDate/localeCompare. |
