...
| P2 | Abierto (detectado 2026-08-21, pre-existente verificado en baseline `c4d71ae`): specs E2E inestables en smoke paralelo: `assets > procesa una imagen`, `nojs-coverage E1/C2`, `ui-sweep-a27 C4/C8/C11`, `scale-store > busca por ancestro` y `catalog-modern-v2 > recupera el carrito / estabilidad visual` fallan intermitentemente bajo carga (8 workers) pero pasan aislados o con reintento; el mismo set falla en el commit base sin los refactors. | Gate smoke puede fallar sin regresión real; dificulta detectar bugs nuevos. | Triaje dedicado: timeouts, espera explícita de red/animaciones y workers reducidos; separar specs lentos (>30s). |
