...
| P2 | Parcialmente resuelto (2026-08-23): draft distingue su runtime con la marca `// DEBUG: modo draft`, production permanece inline byte-idéntico y `scripts/build-runtime.mjs` genera bundle+mapa local; falta emitir `assets/storefront.js.map` desde el export draft y extender `check:runtime-serialization` al entry nuevo. Tests: `packages/exporter/src/runtime-debug.test.ts` y `tests/e2e/__bugs__/runtime-draft.spec.ts`. | El mapa aún no viaja con el sitio draft; debugging requiere paso local manual. | Cerrar Task 5/6 restante del plan `docs/superpowers/plans/2026-08-21-flaky-e2e-runtime-debuggeable.md`: mapa en draft servido sólo en preview y gate de coherencia serializado↔bundle. |
