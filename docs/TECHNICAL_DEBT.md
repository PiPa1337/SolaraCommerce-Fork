...
| P2 | Resuelto (2026-08-23): draft emite `assets/storefront.js` con marca DEBUG + `//# sourceMappingURL=storefront.js.map`, y copia `assets/storefront.js.map` desde `packages/storefront-runtime/dist/` (generado por `scripts/build-runtime.mjs`; fallback `{}` si no corrió). Production permanece inline byte-idéntico sin mapa. `check:runtime-serialization` valida coherencia: 3 tests pre-existentes + probe del entry draft (4/4). Tests: `packages/exporter/src/runtime-debug.test.ts`, `tests/e2e/__bugs__/runtime-draft.spec.ts`. | — | Si se modifica `entry-draft.ts` o el pipeline de build, re-ejecutar `check:runtime-serialization`. |
